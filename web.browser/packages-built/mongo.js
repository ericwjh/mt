...
Mongo.Collection = function (name, options) {                                                                         // 26
  ...
  if (self._connection && self._connection.registerStore) {                                                           // 112
    ...
    var ok = self._connection.registerStore(name, {                                                                   // 116
      ...
      beginUpdate: function beginUpdate(batchSize, reset) {                                                           // 127
      ...
      update: function update(msg) {                                                                                  // 142
      ...
      endUpdate: function endUpdate() {                                                                               // 195
        self._collection.resumeObservers();                                                                           // 196
      },                                                                                                              //
                                                                                                                      //
      // Called around method stub invocations to capture the original versions                                       //
      // of modified documents.                                                                                       //
      saveOriginals: function saveOriginals() {                                                                       // 201
        self._collection.saveOriginals();                                                                             // 202
      },                                                                                                              //
      retrieveOriginals: function retrieveOriginals() {                                                               // 204
        return self._collection.retrieveOriginals();                                                                  // 205
      },                                                                                                              //
                                                                                                                      //
      // Used to preserve current versions of documents across a store reset.                                         //
      getDoc: function getDoc(id) {                                                                                   // 209
        return self.findOne(id);                                                                                      // 210
      },                                                                                                              //
                                                                                                                      //
      // To be able to get back to the collection from the store.                                                     //
      _getCollection: function _getCollection() {                                                                     // 214
        return self;                                                                                                  // 215
      }                                                                                                               //
    });                                                                                                               //
                                                                                                                      //
...
_.extend(Mongo.Collection.prototype, {                                                                                // 242
                                                                                                                      //
  _getFindSelector: function _getFindSelector(args) {                                                                 // 244
    if (args.length == 0) return {};else return args[0];                                                              // 245
  },                                                                                                                  //
                                                                                                                      //
  _getFindOptions: function _getFindOptions(args) {                                                                   // 251
    var self = this;                                                                                                  // 252
   ...
   */                                                                                                                 //
  find: function find() /* selector, options */{                                                                      // 288
   ...
   * @returns {Object}                                                                                                //
   */                                                                                                                 //
  findOne: function findOne() /* selector, options */{                                                                // 313
    var self = this;                                                                                                  // 314
...
Mongo.Collection._publishCursor = function (cursor, sub, collection) {                                                // 322
  var observeHandle = cursor.observeChanges({                                                                         // 323
    added: function added(id, fields) {                                                                               // 324
      sub.added(collection, id, fields);                                                                              // 325
    },                                                                                                                //
    changed: function changed(id, fields) {                                                                           // 327
      sub.changed(collection, id, fields);                                                                            // 328
    },                                                                                                                //
    removed: function removed(id) {                                                                                   // 330
 ...
 */                                                                                                                   //
Mongo.Collection.prototype.insert = function insert(doc, callback) {                                                  // 446
  ...
  var chooseReturnValueFromCollectionResult = function chooseReturnValueFromCollectionResult(result) {                // 446
    if (doc._id) {                                                                                                    // 481
      return doc._id;                                                                                                 // 482
    }                                                                                                                 //
                                                                                                                      //
 ...
 */                                                                                                                   //
Mongo.Collection.prototype.update = function update(selector, modifier) {                                             // 531
 ...
 */                                                                                                                   //
Mongo.Collection.prototype.remove = function remove(selector, callback) {                                             // 588
...
Mongo.Collection.prototype._isRemoteCollection = function _isRemoteCollection() {                                     // 615
 ...
 * @param {Function} [callback] Optional.  If present, called with an error object as the first argument and, if no error, the number of affected documents as the second.
 */                                                                                                                   //
Mongo.Collection.prototype.upsert = function upsert(selector, modifier, options, callback) {                          // 643
