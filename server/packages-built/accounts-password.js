...
var passwordValidator = Match.OneOf(String, { digest: String, algorithm: String });                                    
...
// method previously.                                                                                                  
Meteor.methods({ changePassword: function changePassword(oldPassword, newPassword) {                                   
    check(oldPassword, passwordValidator);                                                                             
    check(newPassword, passwordValidator);                                                                             
                                                                                                                       
    if (!this.userId) throw new Meteor.Error(401, "Must be logged in");                                                
...
Meteor.methods({ forgotPassword: function forgotPassword(options) {                                                    
...
Meteor.methods({ resetPassword: function resetPassword(token, newPassword) {                                           
    var self = this;                                                                                                   
    return Accounts._loginMethod(self, "resetPassword", arguments, "password", function () {                           
      ...
      if (!user) throw new Meteor.Error(403, "Token expired");                                                         
      ...
      var resetToOldToken = function resetToOldToken() {                                                               
...
// and log them in.                                                                                                    
Meteor.methods({ verifyEmail: function verifyEmail(token) {                                                            
    var self = this;                                                                                                   
    return Accounts._loginMethod(self, "verifyEmail", arguments, "password", function () {                             
      ...
      Meteor.users.update({ _id: user._id,                                                                             
        'emails.address': tokenRecord.address }, { $set: { 'emails.$.verified': true },                                
...
Accounts.addEmail = function (userId, newEmail, verified) {                                                            
  ...
  try {                                                                                                                
    ...
    // Undo update if the check fails                                                                                  
    Meteor.users.update({ _id: user._id }, { $pull: { emails: { address: newEmail } } });                              
    throw ex;                                                                                                          
  }                                                                                                                    
};                                                                                                                     
...
//                                                                                                                     
// returns the user id                                                                                                 
var _createUser = function _createUser(options) {                                                                      
...
Meteor.methods({ createUser: function createUser(options) {                                                            
    var self = this;                                                                                                   
...
Accounts.createUser = function (options, callback) {                                                                   
  ...
  return _createUser(options);                                                                                         
};                                                                                                                     
                                                                                                                       
///                                                                                                                    
/// PASSWORD-SPECIFIC INDEXES ON USERS                                                                                 
///                                                                                                                    
Meteor.users._ensureIndex('services.email.verificationTokens.token', { unique: 1, sparse: 1 });                        
Meteor.users._ensureIndex('services.password.reset.token', { unique: 1, sparse: 1 });                                  
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
