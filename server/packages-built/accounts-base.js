 ...
 */                                                                                                                
Accounts = new _accounts_server.AccountsServer(Meteor.server);                                                     
...
AccountsServer = _accounts_server.AccountsServer;                                                                  
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
Ap.addDefaultRateLimit = function () {                                                                             
  if (!defaultRateLimiterRuleId) {                                                                                 
    defaultRateLimiterRuleId = DDPRateLimiter.addRule({                                                            
      ...
      connectionId: function connectionId(_connectionId) {                                                         
        return true;                                                                                               
      }                                                                                                            
    }, 5, 10000);                                                                                                  
  }                                                                                                                
...
var AccountsServer = exports.AccountsServer = function (_AccountsCommon) {                                         
  ...
  function AccountsServer(server) {                                                                                
    ...
    _this._server = server || Meteor.server;                                                                       
    // Set up the server's methods, as if by calling Meteor.methods.                                               
    _this._initServerMethods();                                                                                    
                                                                                                                   
    _this._initAccountDataHooks();                                                                                 
    ...
    _this._autopublishFields = {                                                                                   
      loggedInUser: ['profile', 'username', 'emails'],                                                             
      otherUsers: ['profile', 'username']                                                                          
    };                                                                                                             
    _this._initServerPublications();                                                                               
    ...
    // one was theirs).                                                                                            
    _this._userObservesForConnections = {};                                                                        
    _this._nextUserObserveNumber = 1; // for the number described above.                                           
                                                                                                                   
    // list of all registered handlers.                                                                            
    _this._loginHandlers = [];                                                                                     
                                                                                                                   
    setupUsersCollection(_this.users);                                                                             
    setupDefaultLoginHandlers(_this);                                                                              
    setExpireTokensInterval(_this);                                                                                
                                                                                                                   
    _this._validateLoginHook = new Hook({ bindEnvironment: false });                                               
    _this._validateNewUserHooks = [defaultValidateNewUserHook.bind(_this)];                                        
                                                                                                                   
    _this._deleteSavedTokensForAllUsersOnStartup();                                                                
                                                                                                                   
    _this._skipCaseInsensitiveChecksForTest = {};                                                                  
    return _this;                                                                                                  
  }                                                                                                                
...
//                                                                                                                 
var tryLoginMethod = function tryLoginMethod(type, fn) {                                                           
  var result;                                                                                                      
  try {                                                                                                            
    result = fn();                                                                                                 
  } catch (e) {                                                                                                    
...
Ap._initServerPublications = function () {                                                                         
  ...
  Package.autopublish && Meteor.startup(function () {                                                              
    ...
    var toFieldSelector = function toFieldSelector(fields) {                                                       
...
function defaultResumeLoginHandler(accounts, options) {                                                            
  ...
  // need to.                                                                                                      
  var user = accounts.users.findOne({ "services.resume.loginTokens.hashedToken": hashedToken });                   
...
Ap.config = function (options) {                                                                                   
  ...
  var superResult = _accounts_common.AccountsCommon.prototype.config.apply(this, arguments);                       
...
Ap.insertUserDoc = function (options, user) {                                                                      
  ...
  try {                                                                                                            
    ...
    if (e.err.indexOf('emails.address') !== -1) throw new Meteor.Error(403, "Email already exists.");              
    if (e.err.indexOf('username') !== -1) throw new Meteor.Error(403, "Username already exists.");                 
...
Ap.updateOrCreateUserFromExternalService = function (serviceName, serviceData, options) {                          
  ...
  if (!_.has(serviceData, 'id')) throw new Error("Service data for service " + serviceName + " must include id");  
...
function setupUsersCollection(users) {                                                                             
  ...
  users.allow({                                                                                                    
    ...
    // nothing else.                                                                                               
    update: function update(userId, user, fields, modifier) {                                                      
  ...
  users._ensureIndex('services.resume.loginTokens.hashedToken', { unique: 1, sparse: 1 });                         
  users._ensureIndex('services.resume.loginTokens.token', { unique: 1, sparse: 1 });                               
  // For taking care of logoutOtherClients calls that crashed before the                                           
  // tokens were deleted.                                                                                          
  users._ensureIndex('services.resume.haveLoginTokensToDelete', { sparse: 1 });                                    
...
_accounts_server.AccountsServer.prototype.urls = {                                                                 
  ...
  enrollAccount: function enrollAccount(token) {                                                                   
