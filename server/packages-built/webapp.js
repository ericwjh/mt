...
WebAppInternals.generateBoilerplateInstance = function (arch, manifest, additionalOptions) {                         
  ...
  return new Boilerplate(arch, manifest, _.extend({                                                                  
    pathMapper: function pathMapper(itemPath) { 
      // console.log(__dirname, itemPath, archPath[arch])
      return path.resolve(archPath[arch], itemPath);                                                                    
    },                                                                                                               
    baseDataExtension: {                                                                                             
...
WebAppInternals.staticFilesMiddleware = function (staticFiles, req, res, next) {                                     
  ...
  var maxAge = info.cacheable ? 1000 * 60 * 60 * 24 * 365 : 0;                                                       
...
var runWebAppServer = function runWebAppServer() {                                                                   
  ...
  WebAppInternals.reloadClientPrograms = function () {                                                               
    syncQueue.runTask(function () {                                                                                  
      ...
      var generateClientProgram = function generateClientProgram(clientPath, arch) {                                 
        ...
        var clientJsonPath = clientPath//path.join(__meteor_bootstrap__.serverDir, clientPath);                                  
      ...
      try {                                                                                                          
        ...
        var clientPaths = {
        "web.browser": path.resolve(__dirname, "../../web.browser/program.json")
        }                                           
  ...
  var _suppressConnectErrors = false;                                                                                
  // connect knows it is an error handler because it has 4 arguments instead of                                      
  . go figure.  (It is not smart enough to find such a thing if it's hidden                                      
  ...
  main = function main(argv) {                                                                                       
    ...
    httpServer.listen(localPort, localIp, Meteor.bindEnvironment(function () {                                       
      if (process.env.METEOR_PRINT_ON_LISTEN) console.log("LISTENING"); // must match run-app.js                     
