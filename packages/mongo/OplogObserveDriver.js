...
var _compileProjection = require('../minimongo/projection')
var _modify = require('../minimongo/modify')

var MinMaxHeap = require('../binary-heap/index_server')
var PHASE = {
  QUERYING: "QUERYING",
  FETCHING: "FETCHING",
  STEADY: "STEADY"
...
function OplogObserveDriver(options) {
  var self = this;
  ...
  console.log('selector', selector, projection)

  if (sorter)
    self._sharedProjection = sorter.combineIntoProjection(self._sharedProjection);
  self._sharedProjectionFn = _compileProjection(
    self._sharedProjection);

  self._needToFetch = new _IdMap;
  ...
  forEachTrigger(self._cursorDescription, function (trigger) {
    self._stopHandles.push(self._mongoHandle._oplogHandle.onOplogEntry(
      trigger, function (notification) {
        global._noYieldsAllowed(finishIfNeedToPollQuery(function () {
...
module.exports = OplogObserveDriver
_.extend(OplogObserveDriver.prototype, {
          ...
          self._currentlyFetching.forEach(function (cacheKey, id) {
            ...
            self._mongoHandle._docFetcher.fetch(
              ...
              finishIfNeedToPollQuery(function (err, doc) {
                try {
                  if (err) {
                    console.error("Got exception while fetching documents: " +
                                  err);
  ...
  _runQuery: function (options) {
    ...
    while (true) {
      ...
      newResults = new _IdMap;
      newBuffer = new _IdMap;
      ...
      try {
        ...
        // instead of crashing.
        console.error("Got exception while polling query: " + e);
...
OplogObserveDriver.cursorSupported = function (cursorDescription, matcher) {
  ...
  if (options.fields) {
    try {
      _compileProjection._checkSupportedProjection(options.fields);
    } catch (e) {
      if (e.name === "MinimongoError")
        return false;
      else
