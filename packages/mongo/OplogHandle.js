...
function OplogHandle(oplogUrl, dbName) {
  var self = this;
  ...
  self._baseOplogSelector = {
    ns: new RegExp('^' + _escapeRegExp(self._dbName) + '\\.'),
    $or: [
      { op: {$in: ['i', 'u', 'd']} },
      // drop collection
  ...
  self._entryQueue = new _DoubleEndedQueue();
...
_.extend(OplogHandle.prototype, {
  ...
  onOplogEntry: function (trigger, callback) {
    ...
    callback = global.bindEnvironment(function (notification) {
      // XXX can we avoid this clone by making oplog.js careful?
      originalCallback(EJSON.clone(notification));
    }, function (err) {
      console.error("Error in oplog callback", err.stack);
    });
  ...
  waitUntilCaughtUp: function () {
    ...
    while (!self._stopped) {
      ...
      try {
        ...
        // instead of crashing.
        console.error("Got exception while reading last entry: " + e);
