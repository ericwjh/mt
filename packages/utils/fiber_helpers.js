...
global._noYieldsAllowed = function (f) {
...
// console.log(global._DoubleEndedQueue)
// global._SynchronousQueue is a queue which runs task functions serially.
...
global._SynchronousQueue = function () {
  ...
  self._taskHandles = new _DoubleEndedQueue();
...
_.extend(global._SynchronousQueue.prototype, {
  runTask: function (task) {
    ...
    var fut = new Future;
    var handle = {
      task: global.bindEnvironment(task, function (e) {
        global._debug("Exception from task:", e && e.stack || e);
        throw e;
      }),
      future: fut,
      name: task.name
  ...
  _run: function () {
    ...
    try {
      ...
      if (taskHandle.future) {
        ...
        global._debug("Exception in queued task: " + (err.stack || err));
      }
    }
    self._currentTaskFiber = undefined;
...
// methods).
//
global._sleepForMs = function (ms) {
