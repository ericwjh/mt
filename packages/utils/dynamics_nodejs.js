...
global._nodeCodeMustBeInFiber = function () {
  if (!Fiber.current) {
    console.log('!Fiber.current')
    throw new Error("Meteor code must always run within a Fiber. " +
                    "Try wrapping callbacks that you pass to non-Meteor " +
                    "libraries with global.bindEnvironment.");
  }
};

global.EnvironmentVariable = function () {
  this.slot = nextSlot++;
};

_.extend(global.EnvironmentVariable.prototype, {
  get: function () {
    global._nodeCodeMustBeInFiber();
...
global.bindEnvironment = function (func, onException, _this) {
  global._nodeCodeMustBeInFiber();
  ...
  if (!onException || typeof(onException) === 'string') {
    ...
    throw new Error('onException argument must be a function, string or undefined for global.bindEnvironment().');
  }
