...
var Cursor = function (collection, selector, options) {
  ...
  self.sorter = null;
  self.matcher = new Matcher(selector);

  if (objectid._selectorIsId(selector)) {
    // stash for fast path
    self._selectorId = selector;
  } else if (objectid._selectorIsIdPerhapsAsObject(selector)) {
  ...
  self._projectionFn = _compileProjection(self.fields || {});
...
_.extend(Cursor.prototype, {
  ...
  observe: function (options) {
    var self = this;
    return observe._observeFromObserveChanges(self, options);
  },
  ...
  observeChanges: function (options) {
    ...
    var ordered = observe._observeChangesCallbacksAreOrdered(options);
